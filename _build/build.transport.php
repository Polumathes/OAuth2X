...
$sources = array(
    ...
    'source_api' => $root . 'api/v1/Controllers',
...
$hasApi = is_dir($sources['source_api']); /* Transfer the files in the core dir. */
...
foreach ($categories as $k => $categoryName) {
    ...
    if($hasApi && $i == 1) {
        $helper->sendLog(modx::LOG_LEVEL_INFO, 'packaged API files');
        $vehicle->resolve('file',array(
            'source' => $sources['source_api'],
            'target' => "return MODX_BASE_PATH.'v1/';"
        ));
    }
